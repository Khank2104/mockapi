...
async function checkAuth() {
    ...
    // 3. Logged in locally, but on Login/Register page -> Validate with server to avoid loops
    if (path === '/account/login' || path === '/account/register') {
        try {
            const res = await fetch(`${API_PROXY_URL}/GetMyProfile`);
            if (res.ok) {
